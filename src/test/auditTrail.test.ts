...
describe("queryAuditEvents", () => {
  ...
  it("records tamper-evident audit log for successful unlocks without exposing plaintext (#457)", async () => {
    mockCreate.mockResolvedValueOnce({});

    const walletAddress = "GBBD47IF6LWK7P7MDEVSCWR7DPUWV3NY3DTQEVFL4NAT4AQH3ZLLFLA5";
    const promptId = "1048";
    const requestId = "req-unlock-success-457";
    ...
    // Check required fields (wallet, prompt, timestamp)
    ...
    expect(savedEntry.sequence).toBeGreaterThan(0);
    ...
    expect(savedEntry).not.toHaveProperty("privateKey");
  });
});
